...
{
    public async Task<WorkflowExecutionResult> ExecuteAsync(AutomationWorkflow workflow, IExecutionEventSink sink, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            foreach (var step in workflow.Steps)
            {
                ...
                if (!result.Succeeded)
                {
                    await sink.OnEventAsync(new("failure", module.Descriptor.Id, Message: "Step failed", StepId: step.Id), cancellationToken);
                    return new WorkflowExecutionResult(WorkflowExecutionStatus.Failure, ErrorMessage: $"Step {step.Id} failed");
                }
        ...
        catch (Exception ex)
        {
            ...
            return new WorkflowExecutionResult(WorkflowExecutionStatus.Exception, ErrorMessage: ex.Message, Exception: ex);
        }
    }
}
