...
public sealed class EchoModuleTests
{
    ...
    public async Task EchoReturnsMessageAndRejectsMissingOrEmptyMessage()
    {
        ...
        var result = await module.ExecuteAsync(new(workflow, workflow.Steps[0], workflow.Steps[0].Parameters, CancellationToken.None));
        ...
        foreach (var parameters in new[] { new Dictionary<string, object?>(), new() { ["message"] = null }, new() { ["message"] = " " } })
        {
            ...
            await Assert.ThrowsAsync<ArgumentException>(() => module.ExecuteAsync(new(invalidWorkflow, invalidWorkflow.Steps[0], invalidWorkflow.Steps[0].Parameters, CancellationToken.None)));
        }

        using var cancellation = new CancellationTokenSource();
        cancellation.Cancel();
        await Assert.ThrowsAnyAsync<OperationCanceledException>(() => module.ExecuteAsync(new(workflow, workflow.Steps[0], workflow.Steps[0].Parameters, cancellation.Token)));
    }
}
